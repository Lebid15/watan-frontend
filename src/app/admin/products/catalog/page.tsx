...
export default function AdminCatalogPage() {
  ...
  async function handleEnableProduct(productId: string) {
    setBusyId(productId);
    try {
      const { data } = await api.post<EnableProductResp>(`/admin/catalog/products/${productId}/enable-all`);
      show(`✅ تم التفعيل: جديدة ${data.createdPackages} / متجاهلة ${data.skippedPackages} / المجموع ${data.totalFromCatalog}`);
    } catch (e: any) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <a href="/admin/products/api-settings" className="text-sm text-blue-600 underline">رجوع إلى إعدادات API</a>
      </div>
